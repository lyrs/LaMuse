...
from numpy import kron, ones, zeros, size, shape, os

from LaMuse.tools.MaskRCNNModel import MaskRCNNModel
...
def barycentre(image): #entrée : Image, Sortie : abssice et ordonnée du barycentre
    ...
    for i in range(N):
        for j in range(M):
            if (image[i, j][0] != 0 | image[i, j][1] != 0 | image[i, j][2] != 0 | image[i, j][3] != 0):
                    cptB=cptB+1
                    bar_abs=bar_abs+i
                    bar_ord=bar_ord+j
    print(N, M)
