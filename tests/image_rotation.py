...
from ..tools.compare_images import apply_orientation, best_image, transparancy_mask_to_BW

from ..setup import *


def get_contour(image: np.ndarray):
    img_gray = cv2.cvtColor(transparancy_mask_to_BW(image), cv2.COLOR_BGR2GRAY)
...
if __name__ == "__main__":
    ...
    for i in range(12):
        print("--------------", str(i), "--------------")
        target_image = cv2.imread('./LaMuse/tests/rotate_image_testim/peinture/' + str(i) + '.png', cv2.IMREAD_UNCHANGED)
        print('./LaMuse/tests/rotate_image_testim/peinture/' + str(i) + '.png')
        print(target_image)
    ...
    default_image = cv2.imread('./LaMuse/tests/rotate_image_testim/tigre/2_1.png', cv2.IMREAD_UNCHANGED)
    cd = get_contour(default_image)
    for img in list_images_tigre:
        print("--------------", img, "--------------")
        rotated_image = cv2.imread('./LaMuse/tests/rotate_image_testim/tigre/' + img + '.png', cv2.IMREAD_UNCHANGED)
        finale_image = apply_orientation(get_contour(rotated_image), rotated_image, cd, default_image)
    ...
    default_image = cv2.imread('./LaMuse/tests/rotate_image_testim/helicoptere/186_0d.png', cv2.IMREAD_UNCHANGED)
    cd = get_contour(default_image)
    for img in list_images_helico:
        print("--------------", img, "--------------")
        rotated_image = cv2.imread('./LaMuse/tests/rotate_image_testim/helicoptere/' + img + '.png', cv2.IMREAD_UNCHANGED)
        finale_image = apply_orientation(get_contour(rotated_image), rotated_image, cd, default_image)
