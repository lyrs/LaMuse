...
from LaMuse.tools.barycentre import barycentre
...
def comparaisonGraph(ratioMax: int):
    ...
    for i in range(2,ratioMax):
        ...
        for j in range(len(object_file_list)-1,0,-200):
            ...
            object_image_list.pop(j)
            res_add = res_add + best_image(target_image, object_image_list,0.0, i)[1]

            object_image_list.append(target_image)
        y.append(res_add)
        print("Coucou")
    plt.plot(x,y)
    plt.show()
...
def applyOrientation(contour1, target, contour2, image):
    ...
    if (b_img[0] < image.shape[0] / 2 and b_target[0] > target.shape[0] / 2) or (
            b_img[0] > image.shape[0] / 2 and b_target[0] < target.shape[0] / 2):  # cas gauche / droite
        angleDiff = 180
        # print ("+180")
    elif (b_img[1] < image.shape[1] / 2 and b_target[1] > target.shape[1] / 2) or (
            b_img[1] > image.shape[1] / 2 and b_target[1] < target.shape[1] / 2):
        angleDiff = 180
        # print ("+180")
